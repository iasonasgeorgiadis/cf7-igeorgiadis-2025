...
CREATE INDEX IF NOT EXISTS idx_courses_status ON courses(status) WHERE status = 'active';
...
-- Index for finding active enrollments for reporting
CREATE INDEX IF NOT EXISTS idx_enrollments_status_dates ON enrollments(status, enrolled_at) 
  WHERE status IN ('active', 'completed');

-- Index for finding submissions that need grading
CREATE INDEX IF NOT EXISTS idx_submissions_pending_grading ON submissions(status, submitted_at) 
